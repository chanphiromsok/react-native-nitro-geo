...
open class HybridNitroGeolocationSpec_cxx {
  ...
  // Methods
  @inline(__always)
  public final func requestAuthorization(level: Int32) -> bridge.Result_std__shared_ptr_Promise_AuthorizationResult___ {
    do {
      let __result = try self.__implementation.requestAuthorization(level: margelo.nitro.nitrogeolocation.AuthorizationLevel(rawValue: level)!)
      let __resultCpp = { () -> bridge.std__shared_ptr_Promise_AuthorizationResult__ in
        let __promise = bridge.create_std__shared_ptr_Promise_AuthorizationResult__()
        let __promiseHolder = bridge.wrap_std__shared_ptr_Promise_AuthorizationResult__(__promise)
        __result
          .then({ __result in __promiseHolder.resolve(__result) })
          .catch({ __error in __promiseHolder.reject(__error.toCpp()) })
        return __promise
      }()
      return bridge.create_Result_std__shared_ptr_Promise_AuthorizationResult___(__resultCpp)
    } catch (let __error) {
      let __exceptionPtr = __error.toCpp()
      return bridge.create_Result_std__shared_ptr_Promise_AuthorizationResult___(__exceptionPtr)
    }
  }
